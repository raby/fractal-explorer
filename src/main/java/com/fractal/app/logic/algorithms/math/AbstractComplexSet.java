...
/**
 * This class abstractly represents a set of <code>ComplexNumber</code>s. It 
 * provides state and behaviors which are common to <code>ComplexSet</code>s.
 *
 * @author  Eddie Allen
 *
...
public abstract class AbstractComplexSet implements Set {
    ...
    public AbstractComplexSet(Region region) {
        ...
        this.image = new Image(region.getWidth(), region.getHeight(), region.getId(), region.getX(), region.getY());
    }
    ...
    /**
     * Computes a color value for each pixel representing a point within 
     * the set using <code>BigDecimal</code>s.
     *
     * @param c The point on which to base the set, can be null.
     */
